...
class EMImputationAnalyzer:
    """
    Expectation-Maximization based imputation with multivariate Gaussian assumption
    Implements the complete mathematical framework for metabolomics missing value imputation
    """
    ...
    def _initialize_parameters(self, data):
        ...
        μ^(0) = feature-wise mean of observed values
        Σ^(0) = covariance of observed values + regularization
        """
    ...
    def _conditional_distribution(self, x_obs, obs_idx, miss_idx, mu, sigma):
        """
        Compute conditional distribution of missing values given observed values
        
        Based on conditional Gaussian formula:
        μ_{miss|obs} = μ_miss + Σ_{21}(Σ_{11})^{-1}(x_obs - μ_obs)
        Σ_{miss|obs} = Σ_{22} - Σ_{21}(Σ_{11})^{-1}Σ_{12}
        """
        if len(miss_idx) == 0:
    ...
    def _compute_log_likelihood(self, data, mu, sigma, verbose=False):
        ...
        For each sample, compute contribution to log-likelihood using only observed features
        """
    ...
    def _em_iteration(self, data, mu, sigma, verbose=False):
        ...
        Returns:
            ...
            log_likelihood: observed data log-likelihood
    ...
    def impute_groupwise(self, max_iter=100, tol=1e-4, verbose=True):
        """
        Perform EM imputation separately for each biological group
        """
        if verbose:
            print(f"\n{'='*70}")
            print(f"EM IMPUTATION - GROUP-WISE")
            print(f"{'='*70}")
        ...
        for group, cols in self.groups.items():
            ...
            data_matrix = group_data_filtered.values
            
            if verbose:
                print(f"  Data matrix shape: {data_matrix.shape}")
                print(f"  Initializing parameters...")
            
            mu, sigma = self._initialize_parameters(group_data_filtered)
            
            if verbose:
                print(f"  Initial mean range: [{np.min(mu):.2f}, {np.max(mu):.2f}]")
                print(f"  Initial covariance trace: {np.trace(sigma):.2f}")
        ...
        if verbose:
            print(f"\n{'='*70}")
            print("EM IMPUTATION COMPLETE")
            print(f"{'='*70}\n")
    
    def validate_imputation(self, mask_fraction=0.1, n_trials=5, max_iter=50):
        """
        Validate EM imputation by artificially masking known values
        """
        print(f"\n{'='*70}")
        print(f"EM VALIDATION: {n_trials} trials with {mask_fraction*100}% masking")
        print(f"{'='*70}")
        ...
        for trial in range(n_trials):
            ...
            temp_analyzer = EMImputationAnalyzer(masked_data, self.groups, self.metadata)
    ...
    def plot_convergence(self, output_dir='em_imputation_results'):
        ...
        for idx, (group, history) in enumerate(self.convergence_history.items()):
            ...
            ax.plot(range(len(log_lls)), log_lls, marker='o', linewidth=2, markersize=6)
    ...
    def plot_imputation_comparison(self, output_dir='em_imputation_results'):
        ...
        for idx, (group, cols) in enumerate(self.groups.items()):
            ...
            ax.hist(np.log10(imputed_flat + 1), bins=50, alpha=0.6, 
                   label='After EM', color='green', edgecolor='black')
    ...
    def save_imputed_data(self, output_dir='em_imputation_results', remove_empty_features=True):
        ...
        output_file = f'{output_dir}/imputed_data_em.csv'
    ...
    def generate_report(self, output_dir='em_imputation_results'):
        """Generate comprehensive EM imputation report"""
        Path(output_dir).mkdir(exist_ok=True)
        
        report = []
        report.append("="*70)
        report.append("EM IMPUTATION ANALYSIS REPORT")
        report.append("="*70)
        
        report.append("\nAlgorithm: Expectation-Maximization with Multivariate Gaussian")
        report.append("\nDataset Information:")
        ...
        for group, history in self.convergence_history.items():
            ...
            if len(history['log_likelihoods']) > 0:
                report.append(f"    Initial log-likelihood: {history['log_likelihoods'][0]:.2f}")
                report.append(f"    Final log-likelihood: {history['log_likelihoods'][-1]:.2f}")
                if len(history['log_likelihoods']) > 1:
                    ll_improvement = history['log_likelihoods'][-1] - history['log_likelihoods'][0]
                    report.append(f"    Log-likelihood improvement: {ll_improvement:.2f}")
        ...
        with open(f'{output_dir}/em_imputation_report.txt', 'w', encoding='utf-8') as f:
            f.write(report_text)
        
        print(f"\n[OK] Saved report: {output_dir}/em_imputation_report.txt")
...
def main():
    ...
    print("="*70)
    print("EM IMPUTATION ANALYSIS FOR METABOLOMICS")
    print("="*70)
    ...
    print("\nInitializing EM imputation analyzer...")
    analyzer = EMImputationAnalyzer(abundance_data, groups, metadata)
    
    print("\nStep 1: Validation - Testing EM imputation accuracy")
    analyzer.validate_imputation(mask_fraction=0.1, n_trials=5, max_iter=50)
    
    print("\nStep 2: Performing final EM imputation")
    ...
    print("EM IMPUTATION COMPLETE!")
    print("="*70)
    print("\nNext step: Compare EM vs KNN imputation performance")
